...
- `polaris/issues/` — lightweight bug / incident / observation reports (no scoring; Specs link to Issues they address, never the other way around)
...
1. `/issue` collects four plain fields: What, When, Reproduction, Impact. No PASS/WEAK/FAIL — Issues are operational reports, not strategic commitments. Structure is the quality gate.
2. Optional Goal link. No branch is created; the Issue commits on whatever branch the user is on.
...
- **Issues:** `open/` → `closed/` (fill `Resolution notes`).

All transitions (for Goals, Specs, and Issues) are `git mv` — there is no `status:` field to update. The filesystem is the source of truth.
