---
description: Initialize Polaris in this repository. Runs a setup interview covering context, mission, and 2-3 initial Goals, writes polaris/, injects CLAUDE.md rules, and makes the first commit.
---

# /init

You are executing the Polaris `/init` slash command. This is a setup interview — expect 20-40 minutes of dialogue. Do not shortcut it.
...
> This will initialize Polaris in this repository. I'll interview you in three parts:
...
> 3. **Goals** — 2 to 3 initial Goals for the current phase (each scored across G1–G4)
>
...
## Step 4 — Part C: Initial Goals (iterative, scored)
...
## Step 5 — Anti-strategy consistency check

For each drafted Goal, verify it doesn't violate the Mission's anti-strategy (from B2). If a violation surfaces mid-interview:

- Halt at that Goal. Tell the user: "Goal '{slug}' conflicts with your anti-strategy item '{item}'. You can (a) drop this Goal, (b) revise it so it doesn't conflict, or (c) revise the anti-strategy item. Which?"
- Do not silently proceed. Amending anti-strategy is a strategic decision.
...
2. **State directories** — create empty files:
...
3. **Each Goal** — read `${CLAUDE_PLUGIN_ROOT}/templates/goal.md`, fill with G1–G4 answers and `weak_dimensions`, write to `polaris/goals/active/{timestamp}-{slug}.md`.
...
git commit -m "polaris: initialize mission, {N} goals, CLAUDE.md rules"
```
...
>
> **Written:**
> - `polaris/mission.md` — Mission, anti-strategy ({N} items), phase ({phase}), riskiest assumption
> - `polaris/goals/active/` — {N} Goals: {list of slugs}
...
>
> **Next steps:**
> - Run `/spec` to create your first Spec. You'll be asked which Goal it advances.
> - Run `/goal` later to add more Goals as your thinking sharpens.
